...
- __asserzioni__: avranno lo stesso effetto delle condizioni-if ma perfemmettono di essere considerate nella compilazione in base alla circostanza (utile includerle nel testing ma non nella relase dato che ormai saremo certi di non finire mai in questo caso).
...
Si tratta di un Optional Type, al prof non piace molto come soluzione (perchè? boh, dice qualcosa sul non usarlo come attrributo).