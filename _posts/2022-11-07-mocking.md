...
e di conseguenza poco controllabile.  
L'ideale è avere molti metodi di test piccoli che testino una singola funzionalità.  
...
Si è visto, anche durante i laboratori, come a volte è difficile testare il **system under test** (SUT) perché questo 
risulta essere dipende da altri componenti (**DOC depended-on component**) che non possono essere utilizzati
...
Esistono diversi tipi di Test Double.
...
ci troviamo a doverli creare solo per conformarci alla firma di qualche metodo che dobbiamo chiamare per portare il SUT
...
In questi casi, si può passare come argomento un **dummy object**. Questi oggetti sono quindi una forma degenere di 
Test Double, esistono solo per poter essere passati da un metodo all'altro, non vengono mai utilizzati tuttavia hanno
l'utilità di eliminare la necessità di costruire gli oggetto reale.
