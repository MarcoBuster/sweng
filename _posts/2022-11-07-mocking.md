...
È importante che chi legge i nostri test sia in grado di determinare rapidamente quale comportamento si sta testando, tuttavia
questo può essere molto difficile se i nostri test non sono strutturati in un qualche modo. Un test può essere molto 
confusionario se vengono invocati, senza alcun criterio, diversi comportamenti del **system under test** (SUT), ciò che 
stiamo testando, atti a fare cose diverse: alcuni per impostare lo stato pre-test (fixture) del SUT, altri per esercitare
il SUT e altri ancora per verificare lo stato post-test del SUT.
...
Per aumentare ulteriormente la leggibilità dei nostri test è desiderabile anche fare in modo che ogni metodo di test 
verifichi una e una sola funzionalità. Certo, un metodo di test può essere corretto anche se verifica più funzionalità, 
ma fornirà sicuramente una minore localizzazione delle anomalie rispetto a un gruppo di test che testano singole 
funzionalità, sarà meno leggibile e contraddistinto da una logica più complessa.
...
Come si è visto anche durante i laboratori, può essere difficile testare un SUT se questo dipende da altri componenti 
software che per un motivo o per un altro non possono essere utilizzati durante il testing. 
Questi componenti prendono il nome di **depended-on component** (DOC) e il loro utilizzo nei nostri test è alle volte
impossibile perché questi possono non essere disponibili in questo momento, non restituire i risultati che servono a un 
determinato test (o far in modo che li restituiscano è molto complesso) oppure perché la loro esecuzione avrebbe effetti
collaterali indesiderati. In altri casi ancora semplicemente la nostra strategia di test richiede un maggiore controllo 
o visibilità del comportamento interno del SUT e l'utilizzo di un DOC reale rende ciò difficile.
...
**Test Double** è un termine generico utilizzato per indicare un qualunque oggetto con cui si sostituisce un DOC 
reale a scopo di test.  
Ovviamente, a seconda del tipo di test che stiamo eseguendo, possiamo codificare diversamente il 
comportamento del Test Double. Questo non dovrà affatto comportarsi come il DOC reale, dovrà 
solo fornire le stesse API in modo che la sua presenza risulti essere trasparente al SUT: per il SUT interagire con il 
DOC reale o con il test double deve essere esattamente la stessa cosa. L'utilizzo di test double ci permetterà di rendere
possibili test che sembravano impossibili.
...
Nella maggior parte dei nostri test è necessario fare in modo che il SUT si trovi in uno stato opportuno prima di quella
che è la fase di exercise e per fare ciò, durante la fase di set up, vengono effettuate chiamate ad alcuni suoi metodi.
Questi possono prendere come argomenti degli oggetti che a volte vengono solo memorizzati in variabili d'istanza e mai 
utilizzati nel codice che stiamo testando. Ci troviamo, dunque, a doverli creare solo per conformarci alla firma di qualche 
metodo che dobbiamo chiamare nella fase di set up: la costruzione di questi oggetti può essere non banale e aggiunge una
complessità non necessaria al test.
